...
class Category extends CI_Controller {
    ...
    public function input()
    {
        if ($this->validation() == FALSE) {
            ...
            if ($this->m_get->check('table_category', array('name'=> $this->post_clean('name'))) == 1) {
                $data   = array('status'=> false, 'msg'=>'Category found in the database!');
            } else {
                $query = array(
                    'name' => $this->post_clean('name'),
                    'value'=> slug($this->post_clean('value')) 
                );
                $insert    = $this->m_crud->create('table_category', $query);
                if ($insert == true) {
                    $data = array('status' => true, 'msg'=> 'Category successfully inserted!');
                } else {
    ...
    public function preview()
    {
        ...
        if($id == null) {
            ...
            $get    = $this->m_get->where('table_category', array('id'=> $id))->row();
            if ($get == null) {
    ...
    public function edit()
    {
        if ($this->validation_edit() == FALSE) {
            $data     = array('status'=> false, 'msg'=> 'Form input have a value');
              ...
              $update     = $this->m_crud->update('table_category', $query, array('id'=> $id));
              if ($update == true) {
                $data     = array('status'=> true, 'msg'=> 'Category successfully updated!');
              } else {
                $data     = array('status'=> false, 'msg'=> 'System Error!');
              }
         }
    ...
    public function delete()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $delete     = $this->m_crud->delete('table_category', array('id'=> $id));
            if ($delete == true) {
    ...
    private function validation()
    {
        ...
        $val->set_rules('name', 'Category Name', 'trim|required');
    ...
    {
        $val        = $this->form_validation;
        $val->set_rules('id', 'Category ID', 'trim|required');
        $val->set_rules('name', 'Category Name', 'trim|required');
        $val->set_rules('value', 'Value', 'trim|required');
        return $val->run();
    }
